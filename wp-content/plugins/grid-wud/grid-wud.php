...
Tags: grid, grids, tile, tiles, wp tiles, wp grids, gallery, galleries, widget, widgets, pods, latest post, youtube, vimeo, video, gallery, responsive, slug, shortcode, slugs, post grids, post tiles, post grid, post tile, image grid, filter, image tile, display, list, page, pages, posts, post, query, custom post type
...
// grid-wud style
  function grid_wud_styles() {	
	...
	//Use the grids or tiles
	 ...
	 wp_register_style( 'grid_wud_style_grids', plugins_url('css/grid-wud.css', __FILE__ ), false, '1.0.3' );
	 wp_enqueue_style( 'grid_wud_style_grids' );
	 wp_register_style( 'grid_wud_style_tiles', plugins_url('css/tiles-wud.css', __FILE__ ), false, '1.0.3' );
...
// grid-wud options page (settings menu option)
	function grid_wud_submenu_page() {
		add_menu_page( 'Grid WUD', 'Grid WUD', 'manage_options', 'grid-wud', 'grid_wud_options_notice', plugins_url('images/wud_icon.png', __FILE__ ) );
	}
...
// Color picker and media uploader for admin
	...
	if   ( $hook == "toplevel_page_grid-wud" ) {
		...
		wp_enqueue_style( 'grid_wud_admin_style', plugins_url('css/admin.css', __FILE__ ), false, '1.0.3' );
...
// New fields from version 1.0.5 on 
	function grid_wud_update(){
		...
		if (get_option('grid_wud_size_excerpt')=='') {update_option('grid_wud_size_excerpt', '16');}
...
//Standard values only inserted on activation.
	function grid_wud_activate() {		
		...
		if (get_option('grid_wud_but_bcolor')=='') {update_option('grid_wud_but_bcolor', '#CCC');}
