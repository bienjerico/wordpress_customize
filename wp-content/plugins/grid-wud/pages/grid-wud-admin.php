...
defined( 'ABSPATH' ) or die( 'No script kiddies please!' );
	...
	global $gwfuncs;
		...
		echo '<p><a href="https://wud-plugins.com" class="button-primary" id="grid-pub-wud" target="_blank" title="100% FREE WUD PLUGINS" ><strong><font color="white">CLICK HERE TO DISCOVER<br> MORE WUD FREE PLUGINS</font></strong></a></p>';
	...
	if ( isset($_POST['wud_opt_hidden']) && $_POST['wud_opt_hidden'] == 'Y' ) {
		...
		if ( isset($_POST['grid_wud_bgc1']) && !$_POST['grid_wud_bgc1']=='') {$grid_wud_bgc1 = filter_var($_POST['grid_wud_bgc1'], FILTER_SANITIZE_STRING);} else{$grid_wud_bgc1 ="#FFFFFF";}
		update_option('grid_wud_bgc1', $grid_wud_bgc1);
		
		if ( isset($_POST['grid_wud_bgc2']) && !$_POST['grid_wud_bgc2']=='') {$grid_wud_bgc2 = filter_var($_POST['grid_wud_bgc2'], FILTER_SANITIZE_STRING);} else{$grid_wud_bgc2 ="#FFFFFF";}
		update_option('grid_wud_bgc2', $grid_wud_bgc2);		
		
		if ( isset($_POST['grid_wud_bgc3']) && !$_POST['grid_wud_bgc3']=='') {$grid_wud_bgc3 = filter_var($_POST['grid_wud_bgc3'], FILTER_SANITIZE_STRING);} else{$grid_wud_bgc3 ="#FFFFFF";}
		update_option('grid_wud_bgc3', $grid_wud_bgc3);
		
		if ( isset($_POST['grid_wud_bgc4']) && !$_POST['grid_wud_bgc4']=='') {$grid_wud_bgc4 = filter_var($_POST['grid_wud_bgc4'], FILTER_SANITIZE_STRING);} else{$grid_wud_bgc4 ="#FFFFFF";}
		update_option('grid_wud_bgc4', $grid_wud_bgc4);
		
		if ( isset($_POST['grid_wud_bgc5']) && !$_POST['grid_wud_bgc5']=='') {$grid_wud_bgc5 = filter_var($_POST['grid_wud_bgc5'], FILTER_SANITIZE_STRING);} else{$grid_wud_bgc5 ="#FFFFFF";}
		...
		if($grid_wud_round_index==''){$grid_wud_round_index='1';}
...
// echo'<div id="grid-wud-tip"><b class="grid-trigger" style="float:right; background:#3A6779; color: white;">&nbsp;?&nbsp;</b><div class="tooltip">'.__("tips, help, support and others2", "grid-wud").'</div></div>';
		...
		echo '<label>'.__("Active", "grid-wud").': </label><input class="grid-wud-right" name="grid_wud_fade_in" type="checkbox" value="1" '. checked( $grid_wud_fade_in, "1", false ) .'/><br><br>';
		...
		echo '</select>';		
		echo '<label class="grid-wud-admin-title">'.__("Use a Thumbnail, Medium or Full image in the grid/tile", "grid-wud").'</label>';
		echo '<br><hr>';
...
//RIGHT ADMIN		
		...
		echo '</select>';		
		echo '<label class="grid-wud-admin-title">'.__("At last Grid shown", "grid-wud").'</label><br><hr>';
		...
		echo '</select>';
		echo '<label>'.__("Post/Page or none", "grid-wud").': </label>';	
		echo '<br><br><hr>';
		...
		echo '<label class="cs-wp-color" >'.__("", "grid-wud").'[bg="5"] or [bg="15"] </label><input type="hidden" class="cs-wp-color-picker" name="grid_wud_bgc5" value="'. $grid_wud_bgc5. '" data-rgba="false"><br><br><hr>';		

		echo'<div id="grid-wud-tip"><b class="grid-trigger" style="float:right; background:#3A6779; color: white;">&nbsp;?&nbsp;</b><div class="tooltip">'.__("Check if shortcode is displayed in the widget, instead of the grids/tiles.", "grid-wud").'</div></div>';
		echo '<b class="grid-wud-admin-title">'.__("Widgets shortcode", "grid-wud").'</b><br>';		
		echo '<label>'.__("Active", "grid-wud").': </label><input class="grid-wud-right" name="grid_wud_widgets" type="checkbox" value="1" '. checked( $grid_wud_widgets, "1", false ) .'/><hr>';
		...
		echo '</select>';		
		echo '<label class="grid-wud-admin-title">'.__("Use a Thumbnail, Medium or Full image in the gallery", "grid-wud").'</label>';
		
		
		echo '</div><div class="clear"></div>';
		echo '<div><br>';		
		echo '<input type="submit" name="Submit" class="button-primary" id="grid-wud-adm-subm" value="'.__("Save Changes", "grid-wud").'" /><br><br>';
		echo '<p style="float:right;"><a href="https://wud-plugins.com" class="button-primary" id="grid-pub-wud" target="_blank" title="100% FREE WUD PLUGINS" ><strong><font color="white">CLICK HERE TO DISCOVER<br> MORE WUD FREE PLUGINS</font></strong></a></p><br><br>';
		//Form send
		echo "</form>";
		echo '<a href="https://wud-plugins.com" class="button-primary" id="grid-adm-wud" target="_blank">'.__("Visit our website", "grid-wud").'</a>  <a href="https://wordpress.org/support/plugin/grid-wud" class="button-primary" id="grid-adm-wud" target="_blank">'.__("Get FREE Support", "grid-wud").'</a>';
		echo '</div></div>';		
}
?>